...
void LatestVersionCheckerAndUpdater::run()
{
    ...
    if (info == nullptr)
    {
        if (showAlertWindows)
            AlertWindow::showMessageBoxAsync (AlertWindow::WarningIcon,
                                              ...
                                              "Failed to communicate with the Commsbus update server.\n"
                                              "Please try again in a few minutes.\n\n"
                                              "If this problem persists you can download the latest version of Commsbus from github.com/lifenzdotorg/commsbus");
    ...
    if (! info->isNewerVersionThanCurrent())
    {
        if (showAlertWindows)
            AlertWindow::showMessageBoxAsync (AlertWindow::InfoIcon,
                                              ...
                                              "Your Commsbus version is up to date.");
        return;
    }
    ...
    String requiredFilename ("commsbus-" + info->versionString + "-" + osString + "."); // to end with any extension
...
public:
    ...
    {
        titleLabel.setText ("Commsbus version " + newVersion, dontSendNotification);
        titleLabel.setFont ({ 15.0f, Font::bold });
        titleLabel.setJustificationType (Justification::centred);
        addAndMakeVisible (titleLabel);

        contentLabel.setText ("A new version of Commsbus is available - would you like to download it?", dontSendNotification);
        ...
        appIcon = Drawable::createFromImageData (BinaryData::commsbus_logo_96_png,
                                                  BinaryData::commsbus_logo_96_pngSize);
        lookAndFeelChanged();
                                                       ...
                                                       const String& releaseNotes)
    {
        ...
        options.dialogTitle = "Download Commsbus version " + newVersionString + "?";
...
private:
    ...
    Result install (const MemoryBlock& data)
    {
        ...
        if (zip.getNumEntries() == 0)
            return Result::fail ("The downloaded file was not a valid Commsbus release file!");
        ...
        if (! unzipTarget.folder.getChildFile ("Commsbus").moveFileTo (targetFolder))
...
void restartProcess (const File& targetFolder)
{
    ...
    #if JUCE_MAC
     auto newProcess = targetFolder.getChildFile ("Commsbus.app").getChildFile ("Contents").getChildFile ("MacOS").getChildFile ("Commsbus");
    #elif JUCE_LINUX
     auto newProcess = targetFolder.getChildFile ("Commsbus");
    #endif

    StringArray command ("/bin/sh", "-c", "while killall -0 Commsbus; do sleep 5; done; " + newProcess.getFullPathName().quoted());
   #elif JUCE_WINDOWS
    auto newProcess = targetFolder.getChildFile ("Commsbus.exe");

    auto command = "cmd.exe /c\"@echo off & for /l %a in (0) do ( tasklist | find \"Commsbus\" >nul & ( if errorlevel 1 ( "
                    + targetFolder.getChildFile ("Commsbus.exe").getFullPathName().quoted() + " & exit /b ) else ( timeout /t 10 >nul ) ) )\"";
   #endif
